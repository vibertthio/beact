...
class DrumMachine extends Component {
  constructor() {
    ...
    this.setCurrentChainElementAtLast = this.setCurrentChainElementAtLast.bind(
      this
    );
    this.setCurrentChainElementAtHere = this.setCurrentChainElementAtHere.bind(
      this
    );
    this.setCurrentChainElementIndex = this.setCurrentChainElementIndex.bind(
      this
    );
  ...
  exitPlayRecord() {
    if (this.sequencer.isPlayingRecord === true) {
      ...
      this.keyboard.clearSchedule(
        this.state.keyRecords[this.state.currentPlayingRecordIdx]
      );
      this.setState({
        data,
        currentPlayingRecord: [],
  ...
  editPattern() {
    if (this.state.patternTitle !== '') {
      axios
        .put(`/api/patterns/${this.state.currentPatternId}`, {
          title: this.state.patternTitle,
          content: this.state.data,
        })
        .then(
  ...
  deleteCurrentPattern() {
    if (this.state.currentPatternId !== '') {
      axios
        .delete(`/api/patterns/${this.state.currentPatternId}`)
        .then(
          axios
            .get('/api/patterns')
            .then(res => {
  ...
  renderChain() {
    const {
      drumNoteChain,
      currentPlayingChainElement,
      currentChainElement,
      currentChainElementIndex,
    } = this.state;
    return drumNoteChain.map((chainElement, i) => (
      ...
      >
        ...
        >
          <span>
            {i + 1}
            {currentChainElement !== '' && currentChainElementIndex === i
              ? 'V'
              : ''}
          </span>
        </div>
          ...
          <div>
            ...
            {this.sequencer.isPlayingRecord === false ? (
              ...
              <button
                className={`${styles.icon} ${styles.menuIcon}`}
                onClick={() =>
                  console.log('Exit Playing Record Button clicked')
                }
            ...
            {playing ? (
              <button
                className={`${styles.icon} ${styles.toggleIcon} ${
                  hidden === true ? '' : styles.displayHide
                }`}
              ...
              <div className={styles.colorMenu}>
                {this.sequencer.recording === true ? (
                  ...
                  <div className={`${styles.subGrid}`}>
                    <div
                      className={`${styles.startSeq} ${styles.btnCont}`}
                    ...
                    <div
                      className={`${styles.stopSeq} ${styles.btnCont}`}
                ...
                <div
                  className={`${styles.savePattern} ${styles.btnCont}`}
                  title="Save Pattern"
                >
                  <button
                ...
                <div
                  className={`${styles.exitPattern} ${styles.btnCont}`}
                  title="Exit Pattern Mode"
                >
                  <button
                ...
                <div
                  className={`${styles.editPattern} ${styles.btnCont}`}
                ...
                <div className={`${styles.patternList}`}>
                  ...
                  />
                  <div className={styles.listCont}>{this.renderPatterns()}</div>
                </div>
                <div className={`${styles.chainIcon} ${styles.noFuncRow}`}>
                  <img src={menu6} alt="Chain Icon" />
                </div>
                <div
                  className={`${styles.startChain} ${styles.btnCont}`}
                  title="Start Chain"
                >
                  <button
                ...
                <div
                  className={`${styles.exitChain} ${styles.btnCont}`}
                  title="Exit Chain Mode"
                >
                  <button
                    onClick={() =>
                      console.log('Exit Chain Button Chain clicked')
                    }
                ...
                <div
                  className={`${styles.deleteCurrentChain} ${styles.btnCont}`}
                  title="Delete Current Chain Item"
                >
                  <button
                    onClick={() =>
                      console.log('Delete Current Chain Element Button clicked')
                    }
                    onTouchTap={() => this.deleteCurrentChainElement()}
                  >
                    <img
                      src={menu5}
                      alt="Delete Current Chain Element Button"
                    />
                  </button>
                </div>
               ...
               ${this.sequencer.recording === true ? styles.displayHide : ''}`}
                >
                  ...
                  />
                  <div className={styles.listCont}>{this.renderRecords()}</div>
                </div>
            ...
            <div>
              <input type="text" id="one" onKeyPress={this.detectKeyboard} />
            </div>
          </div>
        )}
      </div>
